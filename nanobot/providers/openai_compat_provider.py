...
class OpenAICompatProvider(LLMProvider):
    ...
    def _parse(self, response: Any) -> LLMResponse:
        ...
        if response_map is not None:
            ...
            # Fallback: some local models embed tool calls in text content
            if not parsed_tool_calls and content:
                import re as _re
                stripped = _re.sub(r"<think>.*?</think>", "", content, flags=_re.DOTALL).strip()
                parsed_tool_calls, extracted_content = OpenAICompatProvider._extract_text_tool_calls(stripped)
                if parsed_tool_calls:
                    content = extracted_content

            # Strip EOS/EOT tokens leaked by some models (e.g. Ministral via llama.cpp)
            if content:
        ...
        if not tool_calls and content:
            import re as _re
            stripped = _re.sub(r"<think>.*?</think>", "", content, flags=_re.DOTALL).strip()
            tool_calls, extracted_content = OpenAICompatProvider._extract_text_tool_calls(stripped)
            if tool_calls:
                content = extracted_content
    ...
    def _extract_text_tool_calls(content: str) -> tuple[list[ToolCallRequest], str | None]:
        ...
        Handles models that emit tool calls in text rather than structured fields:
        Qwen3 (<tool_call>), Nemotron ([TOOL_CALLS]), Python code-blocks, legacy XML, raw JSON.
        Returns (tool_calls, remaining_content). remaining_content is None when the full
        content is a tool-call envelope.
        """
        import re
        ...
        for m in re.finditer(r"<tool_call>(.*?)</tool_call>", content, re.DOTALL):
            try:
                obj = json_repair.loads(m.group(1).strip())
        ...
        if qwen_calls:
            ...
            logger.info("_extract_text_tool_calls: extracted {} Qwen3 tool call(s): {}", len(qwen_calls), [c.name for c in qwen_calls])
            return qwen_calls, preamble

        # Nemotron/Orchestrator format: [TOOL_CALLS]name[ARGS]{json}
        tc_calls = []
        for m in re.finditer(r"\[TOOL_CALLS\]([\w_\-]+)\[ARGS\](\{)", content, re.DOTALL):
            try:
                args = json_repair.loads(content[m.start(2):])
                if not isinstance(args, dict):
                    args = {}
                tc_calls.append(ToolCallRequest(id=_short_tool_id(), name=m.group(1).strip(), arguments=args))
            except Exception:
                pass
        if tc_calls:
            first = re.search(r"\[TOOL_CALLS\]", content)
            preamble = (content[:first.start()].strip() if first else None) or None
            logger.info("_extract_text_tool_calls: extracted {} [TOOL_CALLS] tool call(s): {}", len(tc_calls), [c.name for c in tc_calls])
            return tc_calls, preamble

        # Python code-block: ```python\nfn(key="val")\n```
        import ast as _ast
        py_block = re.search(r"```(?:python)?\s*\n?([\w_]+\(.*?\))\s*\n?```", content, re.DOTALL)
        if py_block:
            call_src = py_block.group(1).strip()
            fn_m = re.match(r"([\w_]+)\((.*)\)$", call_src, re.DOTALL)
            if fn_m:
                try:
                    dummy = f"_f({fn_m.group(2).strip()})"
                    tree = _ast.parse(dummy, mode="eval")
                    arguments = {kw.arg: _ast.literal_eval(kw.value) for kw in tree.body.keywords if kw.arg}  # type: ignore[attr-defined]
                    py_call = ToolCallRequest(id=_short_tool_id(), name=fn_m.group(1), arguments=arguments)
                    logger.info("_extract_text_tool_calls: extracted Python code-block tool call: {}", fn_m.group(1))
                    return [py_call], content[:py_block.start()].strip() or None
                except Exception:
                    pass

        # Legacy XML: <tool_call><function=name><parameter=k>v</parameter>...</function></tool_call>
        xml_match = re.search(r"<tool_call>", content)
        if xml_match:
            calls = []
            xml_body = content[xml_match.start():]
            for fn_m in re.finditer(r"<function=(\w+)>(.*?)(?:</function>|</tool_call>|\Z)", xml_body, re.DOTALL):
                arguments = {p.group(1): p.group(2).strip()
                             for p in re.finditer(r"<parameter=(\w+)>(.*?)</parameter>", fn_m.group(2), re.DOTALL)}
                if arguments:
                    calls.append(ToolCallRequest(id=_short_tool_id(), name=fn_m.group(1), arguments=arguments))
            if calls:
        ...
        if not match:
            return [], content
        try:
            parsed = json_repair.loads(content[match.start():].strip())
        except Exception:
            return [], content
        ...
        if calls:
            logger.info("_extract_text_tool_calls: extracted {} JSON tool call(s): {}", len(calls), [c.name for c in calls])
            return calls, content[:match.start()].strip() or None
        return [], content
